# Runs react_on_rails generators on fresh-rails-install with varying options and puts each result in its own branch.
...
RESULT_TYPES.each do |result_type|
  ...
  task "gen_#{result_type}", [:version] do |_task, args|
    sh %(git checkout fresh-rails-install)
...
task "pull_all", [:version] do |_task, args|
  RESULT_TYPES.each do |result_type|
    ...
    sh %(git checkout #{branch_name})
    message = branch_name
    sh %(hub pull-request -b fresh-rails-install -h shakacode:#{branch_name} -m "#{message}")
  end
